...
import { Loader2, CheckCircle2, XCircle, Timer, Zap, Cuboid, Trophy, ArrowRight, BrainCircuit, Rocket, Settings2, Grid } from 'lucide-react';
...
const GameArena: React.FC<GameArenaProps> = ({ userStats, updateStats }) => {
  ...
  const startMemoryGame = () => {
      setLoading(true);
      // Generate pairs locally for speed
      const cards = generateMemoryGameSet(selectedTopics, 6); // 12 cards
  ...
  const handleCardClick = (card: MemoryCard) => {
      ...
      // Update local state to show flip immediately
  ...
  const checkForMatch = (card1: MemoryCard, card2: MemoryCard) => {
      ...
      if (isMatch) {
          ...
          updateStats({ 
              ...
              xp: userStats.xp + 10,
              gamesPlayed: userStats.gamesPlayed // Consider memory turns or completions? 
          });
          ...
          if (memoryCards.filter(c => !c.isMatched).length <= 2) {
              setTimeout(() => {
                  alert(`כל הכבוד! סיימת את המשחק עם ${score + 50} נקודות!`);
                  setIsPlaying(false);
                  setGameMode(null);
              }, 500);
          }
  ...
  useEffect(() => {
    ...
    if (isPlaying && !loading && !feedback && gameMode !== 'memory') {
  ...
  const handleAnswer = (answer: string) => {
    ...
    const xpGain = isCorrect ? (currentProblem.isChallenge ? 50 : 20) : 0; 
  ...
  הגדירו את סוג המשחק והתחילו לצבור נקודות!
</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            <div onClick={() => startGame('quiz')} className="group relative bg-slate-800 rounded-3xl p-6 border border-slate-700 hover:border-blue-500 transition-all cursor-pointer hover:-translate-y-2 hover:shadow-[0_0_30px_rgba(59,130,246,0.3)] overflow-hidden">
                <BrainCircuit size={48} className="text-blue-500 mb-6 group-hover:scale-110 transition-transform" />
                <h3 className="text-2xl font-black text-white mb-2">חידון בוס</h3>
                <p className="text-slate-400 text-sm">צברו XP בחידון דינמי עם שאלות בוס ואתגרים.</p>
            </div>

            <div onClick={() => startGame('speed')} className="group relative bg-slate-800 rounded-3xl p-6 border border-slate-700 hover:border-red-500 transition-all cursor-pointer hover:-translate-y-2 hover:shadow-[0_0_30px_rgba(239,68,68,0.3)] overflow-hidden">
                <Zap size={48} className="text-red-500 mb-6 group-hover:scale-110 transition-transform" />
                <h3 className="text-2xl font-black text-white mb-2">מרוץ נגד הזמן</h3>
                <p className="text-slate-400 text-sm">60 שניות על השעון. כמה תרגילים תספיקו?</p>
            </div>

            <div onClick={() => startGame('tower')} className="group relative bg-slate-800 rounded-3xl p-6 border border-slate-700 hover:border-emerald-500 transition-all cursor-pointer hover:-translate-y-2 hover:shadow-[0_0_30px_rgba(16,185,129,0.3)] overflow-hidden">
                <Cuboid size={48} className="text-emerald-500 mb-6 group-hover:scale-110 transition-transform" />
                <h3 className="text-2xl font-black text-white mb-2">מגדל השמיים</h3>
                <p className="text-slate-400 text-sm">בנו את הבניין הגבוה ביותר בלי ליפול.</p>
            </div>

            <div onClick={() => startGame('memory')} className="group relative bg-slate-800 rounded-3xl p-6 border border-slate-700 hover:border-pink-500 transition-all cursor-pointer hover:-translate-y-2 hover:shadow-[0_0_30px_rgba(236,72,153,0.3)] overflow-hidden">
                <Grid size={48} className="text-pink-500 mb-6 group-hover:scale-110 transition-transform" />
                <h3 className="text-2xl font-black text-white mb-2">משחק הזיכרון</h3>
                <p className="text-slate-400 text-sm">מצאו זוגות של תרגיל ותוצאה. אימון למוח!</p>
            </div>
        </div>
      </div>
    );
            ...
            <div className="grid grid-cols-3 md:grid-cols-4 gap-4 perspective-[1000px]">
                {memoryCards.map(card => (
                    <div 
                        ...
                        className={`aspect-square relative cursor-pointer transition-transform duration-500 transform-style-3d 
                            ${card.isFlipped || card.isMatched ? '[transform:rotateY(180deg)]' : ''}`}
                        style={{ transformStyle: 'preserve-3d' }}
                    >
                        {/* Front (Hidden) */}
                        <div className="absolute inset-0 bg-slate-800 rounded-xl border-b-4 border-slate-950 flex items-center justify-center backface-hidden shadow-xl">
                            <span className="text-3xl opacity-20">?</span>
                        </div>
                        {/* Back (Visible Content) */}
                        <div className="absolute inset-0 bg-gradient-to-br from-indigo-600 to-purple-600 rounded-xl border-b-4 border-indigo-800 flex items-center justify-center backface-hidden [transform:rotateY(180deg)] shadow-xl">
                             <span className={`font-black ${card.type === 'answer' ? 'text-yellow-300 text-4xl' : 'text-white text-2xl'} dir-ltr`}>
                                 {card.content}
                             </span>
                             {card.isMatched && <CheckCircle2 className="absolute top-2 right-2 text-green-400 w-6 h-6" />}
                        </div>
                    </div>
                ))}
            </div>
  ...
  // --- STANDARD GAME SCREEN (Quiz, Speed, Tower) ---
    ...
    <div className="max-w-4xl mx-auto p-4 md:p-8 text-white">
      ...
      {currentProblem.isChallenge && (
          <div className="animate-pulse bg-red-500/20 border border-red-500 text-red-200 p-4 rounded-xl mb-6 text-center font-black text-xl tracking-widest flex items-center justify-center gap-3 shadow-[0_0_20px_rgba(239,68,68,0.4)]">
              <Zap className="fill-current" /> קרב בוס! (ניקוד כפול) <Zap className="fill-current" />
          </div>
      )}
      ...
      {feedback && gameMode !== 'speed' && (
        <div className={`fixed inset-x-0 bottom-0 p-6 z-50 animate-pop ${feedback === 'correct' ? 'bg-green-600' : 'bg-red-600'}`}>
           <div className="max-w-4xl mx-auto flex items-center text-white">
                ...
                <div className="mr-4">
                    <h3 className="text-2xl font-black uppercase italic">{feedback === 'correct' ? 'מצוין!' : 'לא נורא, נסה שוב!'}</h3>
                    <p className="text-lg opacity-90 font-medium">{currentProblem.explanation}</p>
                </div>
                {feedback === 'incorrect' && (
                    <button onClick={() => fetchProblem(gameMode!)} className="mr-auto bg-white/20 hover:bg-white/30 px-6 py-2 rounded-full font-bold">המשך</button>
                )}
           </div>
        </div>
      )}
